...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['token']) || empty($_POST['token'])) {
        ...
        if (!$tokenDoc || !isset($tokenDoc['idazienda'])) {
            $message = "Accesso negato: token non riconosciuto.";
            $messageType = 'error';
        } else {
            ...
            if (!isset($_FILES['files']) || empty($_FILES['files']['name'][0])) {
                ...
                for ($i = 0; $i < count($_FILES['files']['name']); $i++) {
                    ...
                    if ($insertResult['duplicates'] > 0 || $insertResult['errors'] > 0) {
                        ...
                        if ($insertResult['errors'] > 0) {
                            $msg .= ", {$insertResult['errors']} errori";
                        }
                        $errorMessages[] = $msg;
                    } elseif ($insertResult['inserted'] == 0) {
                        // Caso specifico: zero inseriti senza duplicati/errori
                        $errorMessages[] = "File '{$_FILES['files']['name'][$i]}': 0 record inseriti (possibili duplicati o errori non rilevati)";
                    }
                ...
                if ($successCount > 0) {
                    $message = "Elaborati con successo $successCount file. ";
                    $message .= "Inseriti $totalDocuments documenti in 'upload_$shortIdAzienda'.";
                    $messageType = 'success';
                }
                
                if (!empty($errorMessages)) {
                    $message .= "<br>Errori: " . implode("<br>", $errorMessages);
 ...
 * @return bool True se indice esiste o creato con successo
 */
    ...
    <script>
        ...
        dropZone.addEventListener('drop', (e) => {
            ...
            if (files.length > 0) {
                ...
                alert('Per favore seleziona solo file JSON');
            }
        });
